...
byte map[snakeWidth][snakeHeight] = {0}; // 地图大小  x-y
...
void Move()
{
    map[snake_Grid[snakeLength - 1][0]][snake_Grid[snakeLength - 1][1]] = BLANK; // 清除尾巴
...
void Eat_Food()
{
    ...
    {                      // 如果蛇头碰到食物，就重新投放食物，并且把食物点重置为0
        ...
        foodNum--;
        Gen_Food();
        map[snake_Grid[0][0]][snake_Grid[0][1]] = BLANK; // 去掉食物
    }
}
